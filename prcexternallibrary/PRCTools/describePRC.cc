...
void describeMarkups(BitByBitData& mData)
{
  ...
  for(unsigned int i = 0; i < number_of_leaders; ++i)
  {
    //cout << "describe leader!" << endl;
	   cout << getIndent() << "--Leader--" << endl;
  if(!checkSectionCode(mData,PRC_TYPE_MKP_Leader))
    return;
  indent();

  describeContentPRCBaseWithGraphics(mData,true);
unsigned int linked_item = mData.readUnsignedInt();
if(linked_item)
{
	unsigned int refprc = mData.readUnsignedInt();
	unsigned int type = mData.readUnsignedInt();

	cout << getIndent() << "reference_in_same_file_structure "
      << (mData.readBit()?"yes":"no") << endl;
...
void describeName(BitByBitData &mData)
{
  ...
  if(!sameNameAsCurrent)
    currentPRCName = mData.readString();
  cout << getIndent() << "Name \"" << currentPRCName << '\"' << endl;
}
