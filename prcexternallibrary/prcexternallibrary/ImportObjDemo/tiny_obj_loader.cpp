  ...
  const char* mtl_basepath)
{
  ...
  while (ifs.peek() != -1) {
    ...
    if (token[0] == 'v' && isSpace((token[1]))) {
	  ...
	  {
		  i++;
		  bbox.boxMax.x = bbox.boxMin.x = x;
		  bbox.boxMax.y = bbox.boxMin.y = y;
		  bbox.boxMax.z = bbox.boxMin.z = z;
	  }

	  if (x > bbox.boxMax.x)  	bbox.boxMax.x = x;
	  if (y > bbox.boxMax.y)	bbox.boxMax.y = y;
	  if (z > bbox.boxMax.z)	bbox.boxMax.z = z;

	  if (x < bbox.boxMin.x)  	bbox.boxMin.x = x;
	  if (y < bbox.boxMin.y)	bbox.boxMin.y = y;
	  if (z < bbox.boxMin.z)	bbox.boxMin.z = z;
      continue;
	}
